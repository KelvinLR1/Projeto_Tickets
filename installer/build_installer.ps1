...
# Controlador
& $venvPython -m PyInstaller --onefile --noconsole --name TicketFlow_Controller --uac-admin --icon="$iconPath" --collect-all pywin32 --hidden-import pywintypes --exclude-module libcrypto --exclude-module libssl --distpath "$pyiDist" --workpath "$pyiWork" --specpath installer scripts/controller.py
