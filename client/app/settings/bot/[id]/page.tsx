...
export default function BotConfigPage() {
    ...
    if (loading) {
        return (
            <main className="h-screen flex items-center justify-center bg-[#07090e] text-white">
    ...
    return (
        <div className="h-screen w-full flex flex-col overflow-hidden bg-[#050608] text-slate-100 select-none relative font-sans">
            {/* Header Superior */}
            <header className="h-16 border-b border-white/[0.04] bg-[#090b11]/70 backdrop-blur-xl flex items-center justify-between px-6 z-30 relative shrink-0">
                <div className="flex items-center gap-4">
                    <Link 
                        href="/settings"
                        className="p-2.5 bg-white/[0.02] hover:bg-white/5 border border-white/5 hover:border-white/10 rounded-xl transition-all duration-300 text-slate-400 hover:text-white shrink-0 active:scale-95"
                    >
                        <ArrowLeft className="w-4 h-4" />
                    </Link>
                    <div>
                        <div className="text-xs font-black uppercase tracking-wider flex items-center gap-2 text-slate-200">
                            Fluxo do Bot 
                            <span className="px-2 py-0.5 text-[9px] font-black tracking-widest uppercase bg-gradient-to-r from-violet-500/10 to-indigo-500/10 border border-violet-500/20 text-violet-300 rounded-lg shadow-sm shadow-violet-500/5">
                                {channel?.name || 'Carregando...'}
                            </span>
                        </div>
                        <p className="text-[9px] text-slate-400 uppercase tracking-widest mt-1">Construa o roteamento automático de clientes</p>
                    </div>
                </div>

                <div className="flex items-center gap-3">
                    {connectingSource && (
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-violet-500/10 border border-violet-500/20 text-violet-400 text-[9px] font-black uppercase tracking-wider rounded-xl animate-pulse">
                            <AlertCircle className="w-3.5 h-3.5" />
                            Selecione uma entrada para conectar
                            <button onClick={() => setConnectingSource(null)} className="ml-1 hover:text-white"><X className="w-3 h-3" /></button>
                        </div>
                    )}
                    
                    <button
                        onClick={startFlowTest}
                        className="px-4 py-2.5 bg-violet-500/10 hover:bg-violet-500/20 border border-violet-500/20 text-violet-400 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center gap-1.5 duration-300 hover:shadow-lg hover:shadow-violet-500/5"
                    >
                    ...
                    <button
                        ...
                        className="px-4 py-2.5 bg-white/[0.02] hover:bg-white/5 border border-white/5 text-slate-400 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 duration-300"
                    >
                        Limpar
                    </button>
                    ...
                    <button
                        ...
                        disabled={saving}
                        className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-violet-500/10 active:scale-95 disabled:opacity-50 duration-300"
                    >
            ...
            <div className="h-14 bg-[#07090e]/40 border-b border-white/[0.04] flex items-center px-6 gap-3.5 z-20 shrink-0 relative backdrop-blur-md overflow-x-auto custom-scrollbar">
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mr-2 shrink-0">Adicionar Bloco:</span>
                
                <button 
                    onClick={() => handleAddNode('message')} 
                    className="flex items-center gap-2 px-4 py-2 bg-violet-500/5 hover:bg-violet-500/10 border border-violet-500/15 hover:border-violet-500/30 text-violet-400 hover:text-violet-300 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all active:scale-95 duration-200 shrink-0"
                >
            ...
            <div 
                ...
                className={clsx(
                    "flex-1 overflow-hidden bg-[#040507] relative z-10 select-none",
            ...
            >
                ...
                <div 
                    className={clsx(
                        "w-[3000px] h-[2000px] absolute top-0 left-0 bg-[radial-gradient(rgba(255,255,255,0.035)_1.5px,transparent_1.5px)] [background-size:24px_24px] canvas-background",
                ...
                >
                    ...
                    <svg className="absolute inset-0 pointer-events-none w-full h-full z-0">
                        ...
                        {edges.map(edge => {
                            ...
                            // Calcula curva bezier cúbica para ficar arredondada e suave
                            ...
                            return (
                                <g key={edge.id} className="group pointer-events-auto cursor-pointer" onClick={() => {
                                    if(confirm('Excluir esta conexão?')) {
                                        setEdges(prev => prev.filter(e => e.id !== edge.id));
                                    }
                                }}>
                                    {/* Linha invisível maior para facilitar o clique */}
                                    <path 
                                        d={d} 
                                        fill="none" 
                                        stroke="transparent" 
                    ...
                    {nodes.map(node => {
                        ...
                        return (
                            <div
                                ...
                                className={clsx(
                                    "absolute w-[260px] bg-slate-950/70 border rounded-3xl shadow-2xl select-none z-10 flex flex-col backdrop-blur-md group",
                                    draggingNodeId !== node.id && "transition-[border-color,transform,box-shadow,ring] duration-200",
                                    isSelected 
                                        ? "border-violet-500 ring-2 ring-violet-500/20 scale-[1.01]" 
                            ...
                            >
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-wider text-slate-100">{node.title}</span>
                                    </div>
                                    {node.id !== 'node-start' && (
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); handleDeleteNode(node.id); }}
                                            className="p-1 hover:bg-red-500/10 border border-transparent hover:border-red-500/20 text-slate-500 hover:text-red-400 rounded-lg transition-all"
                                            title="Excluir Bloco"
                                        >
                                ...
                                {node.type !== 'start' && (
                                    <div 
                                        ...
                                        className={clsx(
                                            "handle-connector absolute -left-2.5 top-13 w-4.5 h-4.5 rounded-full border bg-slate-950 flex items-center justify-center cursor-pointer transition-all z-20",
                                            ...
                                            hoveredInletNodeId === node.id 
                                                ...
                                                : (isConnecting ? "border-violet-500 animate-pulse" : "border-white/20 hover:border-violet-500")
                                        )}
                                ...
                                <div className="p-4 flex-1 flex flex-col justify-center leading-normal text-left select-none text-[10px]">
                                    ...
                                    {node.type === 'message' && (
                                        <p className="text-slate-300 line-clamp-3 font-medium leading-relaxed">{node.data.text}</p>
                                    )}
                                    {node.type === 'question' && (
                                        <div className="space-y-2">
                                            <p className="text-slate-300 font-bold truncate">{node.data.text}</p>
                                                ...
                                                {(node.data.options || []).map((opt, idx) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div 
                                                            ...
                                                            className={clsx(
                                                                "handle-connector absolute -right-4.5 top-1 w-4 h-4 rounded-full border bg-slate-950 flex items-center justify-center cursor-pointer transition-all z-20",
                                                                "opacity-0 pointer-events-none scale-75 group-hover:opacity-100 group-hover:pointer-events-auto group-hover:scale-100 border-white/20 hover:border-blue-500 hover:scale-125"
                                                            )}
                                                        >
                                            ...
                                            <div className="space-y-2 relative">
                                                ...
                                                <div className="flex items-center justify-between px-2 py-1 bg-green-500/5 border border-green-500/10 rounded-md text-[9px] font-black uppercase tracking-wider text-green-400">
                                                    ...
                                                    <div 
                                                        ...
                                                        className={clsx(
                                                            "handle-connector absolute -right-4.5 top-1 w-4 h-4 rounded-full border bg-slate-950 flex items-center justify-center cursor-pointer transition-all z-20",
                                                            "opacity-0 pointer-events-none scale-75 group-hover:opacity-100 group-hover:pointer-events-auto group-hover:scale-100 border-white/20 hover:border-green-500 hover:scale-125"
                                                        )}
                                                    >
                                                ...
                                                <div className="flex items-center justify-between px-2 py-1 bg-red-500/5 border border-red-500/10 rounded-md text-[9px] font-black uppercase tracking-wider text-red-400 mt-1.5">
                                                    ...
                                                    <div 
                                                        ...
                                                        className={clsx(
                                                            "handle-connector absolute -right-4.5 top-8 w-4 h-4 rounded-full border bg-slate-950 flex items-center justify-center cursor-pointer transition-all z-20",
                                                            "opacity-0 pointer-events-none scale-75 group-hover:opacity-100 group-hover:pointer-events-auto group-hover:scale-100 border-white/20 hover:border-red-500 hover:scale-125"
                                                        )}
                                                    >
                                    ...
                                    {node.type === 'http' && (
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-[9px] font-mono text-slate-400 truncate">{node.data.url}</p>
                                ...
                                {node.type !== 'question' && node.type !== 'condition' && (
                                    <div 
                                        ...
                                        className={clsx(
                                            "handle-connector absolute -right-2.5 top-[52px] w-4 h-4 rounded-full border bg-slate-950 flex items-center justify-center cursor-pointer transition-all z-20",
                                            "opacity-0 pointer-events-none scale-75 group-hover:opacity-100 group-hover:pointer-events-auto group-hover:scale-100 border-white/20 hover:border-violet-500 hover:scale-125"
                                        )}
                ...
                <div className="absolute left-6 bottom-6 bg-[#090b11]/85 border border-white/[0.05] rounded-2xl p-2 flex items-center gap-2.5 z-30 shadow-2xl backdrop-blur-xl transition-all hover:border-white/10">
                    <button 
                        onClick={() => adjustZoomAtPoint(-1, undefined, undefined, true)}
                        className="w-7 h-7 rounded-xl bg-white/[0.02] hover:bg-white/5 border border-white/5 flex items-center justify-center text-xs text-slate-300 hover:text-white transition-all font-black active:scale-95"
                        title="Diminuir Zoom"
                    >
                        -
                    </button>
                    <span className="text-[10px] font-black text-slate-300 tracking-wider w-10 text-center select-none font-mono">
                        {Math.round(zoom * 100)}%
                    </span>
                    <button 
                        onClick={() => adjustZoomAtPoint(1, undefined, undefined, true)}
                    ...
                    <button 
                        onClick={resetZoom}
                        className="px-2.5 py-1.5 rounded-xl bg-white/[0.02] hover:bg-white/5 border border-white/5 flex items-center justify-center text-[8px] font-black uppercase tracking-wider text-slate-400 hover:text-white transition-all active:scale-95"
            ...
            <AnimatePresence>
                {selectedNode && (
                    <motion.aside
                        ...
                        className="fixed right-0 top-0 h-screen w-[380px] bg-[#090b11]/95 border-l border-white/5 shadow-2xl z-40 p-6 flex flex-col gap-6 backdrop-blur-xl"
                    >
                        <div className="flex items-center justify-between border-b border-white/5 pb-4">
                            <div>
                                <h3 className="text-xs font-black uppercase tracking-wider text-slate-200">Configurar Bloco</h3>
                                <p className="text-[8px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">Defina as ações e propriedades</p>
                            </div>
                            <button 
                                onClick={() => setSelectedNodeId(null)}
                                className="p-2 hover:bg-white/5 border border-white/5 hover:border-white/10 rounded-xl transition-all"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto pr-1 space-y-6 custom-scrollbar text-xs">
                            {/* Nome do Bloco */}
                            <div className="space-y-2">
                                <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Identificação do Bloco</label>
                                <input
                                    type="text"
                                    ...
                                    className="w-full bg-[#050608] border border-white/5 focus:border-violet-500 rounded-xl p-3.5 font-bold outline-none text-slate-200"
                                />
                            </div>
                            ...
                            {selectedNode.type === 'message' && (
                                <div className="space-y-2">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Mensagem de Texto</label>
                                    <textarea
                                        rows={6}
                                        ...
                                        className="w-full bg-[#050608] border border-white/5 focus:border-violet-500 rounded-xl p-3.5 font-medium outline-none text-slate-200 leading-relaxed resize-none"
                                    />
                                </div>
                            )}
                            ...
                            {selectedNode.type === 'question' && (
                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Texto da Pergunta</label>
                                        <textarea
                                            rows={3}
                                            value={selectedNode.data.text || ''}
                                            onChange={(e) => {
                                            ...
                                            className="w-full bg-[#050608] border border-white/5 focus:border-violet-500 rounded-xl p-3.5 font-bold outline-none text-slate-200 resize-none"
                                        />
                                    </div>

                                    <div className="space-y-3">
                                        <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400 flex items-center justify-between">
                                            Opções de Resposta
                                            <button
                                                onClick={() => {
                                                ...
                                                className="text-violet-400 hover:text-violet-300 font-black text-[9px] flex items-center gap-1 uppercase tracking-widest"
                                            >
                                            ...
                                            {(selectedNode.data.options || []).map((opt, idx) => (
                                                <div key={idx} className="flex items-center gap-2">
                                                    <input
                                                        ...
                                                        className="flex-1 bg-[#050608] border border-white/5 focus:border-violet-500 rounded-xl px-3 py-2 text-xs font-bold outline-none"
                                                    />
                                                    <button
                                                        onClick={() => {
                                                        ...
                                                        className="p-2 hover:bg-red-500/10 border border-white/5 hover:border-red-500/20 text-slate-400 hover:text-red-400 rounded-lg transition-colors shrink-0"
                                                    >
                                                        <Trash2 className="w-3.5 h-3.5" />
                                                    </button>
                                                </div>
                            ...
                            {selectedNode.type === 'condition' && (
                                <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Início do Expediente</label>
                                            <input
                                                type="time"
                                                ...
                                                className="w-full bg-[#050608] border border-white/5 rounded-xl p-3 font-bold outline-none"
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Fim do Expediente</label>
                                            <input
                                                type="time"
                                    ...
                                    <div className="space-y-2.5">
                                        <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Dias de Funcionamento</label>
                                                ...
                                                return (
                                                    <label key={idx} className="flex items-center gap-3.5 cursor-pointer py-0.5">
                                                        <input
                                                            ...
                                                            className="w-4 h-4 rounded border-white/10 text-violet-600 focus:ring-violet-500/20 bg-slate-950"
                                                        />
                            ...
                            {selectedNode.type === 'sector' && (
                                <div className="space-y-2">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Setor de Direcionamento</label>
                                    <select
                                        ...
                                        className="w-full bg-[#050608] border border-white/5 rounded-xl p-3.5 font-bold outline-none text-slate-200"
                                    >
                                        {sectors.map(s => (
                                            <option key={s.id} value={s.id} className="bg-slate-950 text-slate-200">{s.name}</option>
                                        ))}
                                    </select>
                                    <p className="text-[10px] text-slate-500 mt-1">Ao atingir este ponto, o chatbot finaliza e o cliente entra na fila do departamento selecionado.</p>
                                </div>
                            )}
                            ...
                            {selectedNode.type === 'http' && (
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Método HTTP</label>
                                        <select
                                            ...
                                            className="w-full bg-[#050608] border border-white/5 rounded-xl p-3.5 font-bold outline-none text-slate-200"
                                        >
                                            <option value="GET" className="bg-[#050608]">GET</option>
                                            <option value="POST" className="bg-[#050608]">POST</option>
                                        </select>
                                    </div>
                                    <div className="space-y-2">
                                        <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Webhook URL</label>
                                        <input
                                            type="text"
            ...
            <AnimatePresence>
                {isTestChatOpen && (
                    <motion.div
                        ...
                        transition={{ duration: 0.3, ease: 'easeOut' }}
                        className="fixed right-4 bottom-4 top-20 w-[360px] bg-[#090b11]/95 border border-white/5 shadow-2xl rounded-3xl z-40 p-5 flex flex-col backdrop-blur-xl"
                    >
                        {/* Chat Header */}
                        <div className="flex items-center justify-between border-b border-white/5 pb-3 shrink-0">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-[8px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">Visualização em tempo real</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <button 
                                    onClick={startFlowTest}
                                    className="p-2 hover:bg-white/5 border border-white/5 rounded-xl transition-all text-slate-400 hover:text-white"
                                ...
                                <button 
                                    ...
                                    className="p-2 hover:bg-white/5 border border-white/5 rounded-xl transition-all"
                                >
                            ...
                            {testMessages.map((msg, idx) => (
                                <div 
                                    ...
                                    className={clsx(
                                        ...
                                        msg.sender === 'bot' ? "bg-white/5 text-slate-100 self-start border border-white/5" :
                                        msg.sender === 'client' ? "bg-violet-600 text-white self-end shadow-md shadow-violet-500/10" :
                                        "bg-slate-900/50 text-slate-500 border border-white/5 self-center text-center text-[9px] font-black uppercase tracking-widest max-w-full"
                                    )}
                                >
                                    {msg.text}
                                </div>
                        ...
                        {testActiveOptions.length > 0 && (
                            <div className="border-t border-white/5 pt-3.5 space-y-2 shrink-0">
                                <p className="text-[8px] text-slate-500 font-black uppercase tracking-widest mb-2.5">Escolha uma opção:</p>
                                <div className="grid grid-cols-1 gap-2">
                                    {testActiveOptions.map((opt, idx) => (
                                        <button
                                            key={idx}
                                            onClick={() => handleSelectTestOption(opt)}
                                            className="w-full text-left px-4 py-3 bg-white/3 hover:bg-violet-500/10 border border-white/5 hover:border-violet-500/20 text-slate-300 hover:text-violet-400 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all duration-200"
                                        >
                                            {idx + 1}. {opt}
                                        </button>
                                    ))}
